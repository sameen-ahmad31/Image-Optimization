 ...
 */
team_t team = {
    "sameen_ahmad",              /* Team name is where the GW ID goes */

    "Sameen Ahmad",     /* First member full name */
    "sameen_ahmad@gwu.edu",  /* First member email address */
...
/* below are the main computations for your implementation of the rotate. Any changes in implementation will go here or the other functions it may call */
	...
	//declare three variables that will be used besides i and j
	//k will be used as an index for the src array
	//index will be used to loop through 32 times, since it was given that N is a multiple of 32
	//d will be used to calculate the results for the fucnction RIDX that is inlined
	int k, index, z;
	int d = (dim * dim - dim);
	//increased by 32 each time since N is a multiple of 32
	for(j = 0; j < dim; j += 32)
	{
		//loop through until it reaches dim
		for(i = 0; i < dim; i++)
		{
			//the function RIDX was inlined for code optimization so d calculates what the function used to perform
			z = (d - i * dim) + j;
			//k is used to calculate the value that RIDX was used for the src's index
			k = j * dim + i;
			//this loop is used for the array to loop through 32 times, it does better code optimization and the array is accessed in a better way 
			for(index = 0; index < 32; index++)
			{
				//since rotate is transpose and exchange over the rows, for each pair given, it interchanges them
				//k increased by the dim each time so that the interchange happens correctly since dst's index is also increasing each time
				dst[z + index] = src[k];
				//transpose so that M(i,j) and M(j,i) are interchanged
				//row i is exchanged with row N -1 - i
				k += dim;
			}
		}
	} 
